                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="card card-primary">
                            <div class="card-body">
                                <form action="{{ route('0.ubah-foto') }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    ...
                                    <div class="text-center thumbnail-image" onclick="updateImage()">
                                        ...
                                        <input type="file" name="photo" id="update-image-admin" style="display: none"
                                        ...
                                        <input type="file" name="avatar" id="update-image-admin" style="display: none"
                                            onchange="form.submit()">
...
@include('pages.admin.modal-ubah-profile')
@endsection

@push('after-scripts')
<script>
    function updateImage() {
            document.getElementById('update-image-admin').click();
        }

        function ubahProfile(id){
            $('#ubahProfileModal').modal('show');
            $.ajax({
                url: "{{ route('0.get-akun') }}",
                type: "POST",
                data: {
                    id: id,
                    _token: "{{ csrf_token() }}"
        ...
        $('#form-ubah-profile-admin').submit(function(e){
            e.preventDefault();
            var data = new FormData(this);
            $.ajax({
                url: "{{ route('0.update-akun') }}",
                type: "POST",
                data: data,
                contentType: false,
                processData: false,
